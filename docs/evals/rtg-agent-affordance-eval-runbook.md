...
`docs/evals/rtg-individual-life-graph-beta-prompt.md` after the first call succeeds. Use
`docs/evals/rtg-beta-known-good-walkthrough.md` as the expected shape of a successful run.
...
`docs/evals/rtg-agent-affordance-eval-prompt.md` after the MCP server is connected. Prefer the
sequenced prompts first while the RTG tool surface is still new.

For the initial individual open-source use case, use
`docs/evals/rtg-individual-life-graph-beta-prompt.md`. That prompt asks the agent to model one
