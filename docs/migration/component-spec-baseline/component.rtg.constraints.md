...
- Rejects snapshot records without a concrete UUID and rejects duplicate UUIDs rather than replacing an earlier snapshot record.
...
- Lists constraint definitions whose `target_type_keys` set contains `target_type_key`, optionally filtered by constraint kind and live status; a definition with several target keys is returned for each of them.
...
- `target_type_keys` is a unique, unordered set of schema type keys the constraint is primarily about for lookup and discovery. Duplicate inputs are rejected; realization encodings may use a canonical order without giving order domain meaning.
