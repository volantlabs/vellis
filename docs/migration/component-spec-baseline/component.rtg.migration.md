...
status: accepted
owner: humans
model: model/bibliotek/components/component.rtg.migration.sysml
code:
  roots:
...
- Requires every snapshot record to have a concrete unique migration ID and validates all records
  before exposing the reconstructed store.
- Validates lifecycle values, JSON metadata, membership/replacement structure, and evidence
  identities. Snapshot construction does not apply transitions between independent records.
...
- When the migration ID is supplied, the component uses it unchanged to create or fully replace
  that record. Replacing an existing ID must preserve status or follow the same lifecycle transition
  table as `set_status`; rejection has no effect.
...
- Validates that make-live and make-non-live membership is duplicate-free and disjoint per resource
  category, then returns it in UUID order.
- Validates that replacement mappings are one-to-one, never self-replacements, and map an old
  make-non-live identity to a new make-live identity. Additions and removals without replacements
  remain valid.
...
- Setting the status to its current value is allowed. Every accepted call replaces
  `metadata.status_metadata` with the supplied JSON object or `{}` and preserves unrelated metadata.
...
- Membership fields are logical sets encoded in ascending UUID order. Replacement fields are
  one-to-one mappings encoded by old then new UUID.
...
- The old and new IDs differ; old IDs occur in the corresponding make-non-live set, new IDs occur
  in make-live, and neither side is reused in another replacement of that resource category. The
  migration component does not verify that referenced resources exist.
