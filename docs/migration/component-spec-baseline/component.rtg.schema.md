...
- Rejects missing identities, duplicate identities, malformed record encodings, and invalid nested payloads rather than coercing or discarding them.
...
- `definition.payload` must use the payload family selected by the definition kind; a family mismatch is `RtgSchemaDefinitionInvalid`, while malformed contents of the selected payload are `RtgSchemaPayloadInvalid`.
...
- Returns each set in ascending textual type-key order.
...
- `either` is an input selection value. A returned participation may report `both` when the queried type is admitted at both endpoints; `both` is not an input direction.
...
- `anchor_type_keys` is a non-empty unique ordered selection; duplicate keys are rejected and selected anchor schemas preserve request order.
...
- Each collection is a unique unordered set and the two sets are disjoint. Python and JSON representations use ascending textual order.
...
- `value_kinds` is a non-empty unique unordered set containing supported v1 value kinds: `string`, `integer`, `number`, `boolean`, `null`, `object`, `list`, or `uuid`.
...
- Validation kinds are predicates, not mutually exclusive tags: an integer satisfies `integer` and `number`; a fractional number satisfies `number`; every string satisfies `string`; and a canonical UUID string also satisfies `uuid`. Booleans never satisfy numeric kinds.
- Strictness applies recursively: nested object values may contain only properties declared by their nested field schema.
...
- Each endpoint collection is a non-empty unique unordered set represented canonically in ascending textual order. A type may appear in both sets.
...
- Each list is the ascending textual representation of its logical set.
...
- Source and target type lists are ascending textual representations of the link schema payload sets.
...
Definition lists and snapshots use ascending textual UUID order. Associated-data type-key results use ascending textual type-key order. Link participation uses link type key and textual definition UUID; endpoint type-key lists are ascending. Schema packs preserve unique requested anchor order and use definition order for their remaining collections.
