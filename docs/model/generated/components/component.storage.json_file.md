...
| `contract.storage.json_file.list_effect` | `ListJsonDocuments` | `storage.list` | Success recursively returns metadata for regular .json documents below the requested directory, never parsed values or files outside the root, and leaves storage state unchanged. |
