...
|---|---|---|---|---|
| `applyLiveGraphChanges` | `ApplyLiveGraphChanges` | in `graphChanges: RtgGraphChangeSet`; in `validationMode: RtgControllerValidationMode` = `RtgControllerValidationMode::strict`; out `result: RtgControllerOperationResult` | `RtgControllerValidationFailed`, `RtgControllerWriteConflict`, `RtgControllerApplyFailed` | Validate and apply normal live graph CRUD in the serialized mutation lane. |
...
| `getObject` | `ControllerGetObject` | in `objectUuid: String`; out `result: RtgControllerObjectRead` | `RtgControllerObjectNotFound` | Normalize UUID text and return the public graph record without lifecycle filtering, plus concurrency and audit context; invalid and absent UUIDs are reported through the controller-owned not-found contract. |
...
| `applyMigrationCutover` | `derivePlan: local`, `validateProjection: ValidateRtgChangeBatch`, `capturePreimage: local`, `applySchemaEvolutionDataEffects: local`, `applyCutover: local`, `validateActualState: ValidateRtgChangeBatch`, `commitApplied: local`, `restorePreimage: local`, `recordFailed: local` | `first derivePlan then validateProjection;`; `first capturePreimage then applySchemaEvolutionDataEffects;`; `first applySchemaEvolutionDataEffects then applyCutover;`; `first restorePreimage then recordFailed;` |
...
| `contract.rtg.controller.kernel_meta_model_apply` | `ApplyLiveGraphChanges` | `controller.applyLiveGraphChanges` | Before graph mutation, the controller preserves identity overrides in the resolved request and ledger evidence, blocks strict merge candidates unless force_create converted them to informational evidence, and assigns kernel-owned created_at and updated_at timestamps. New records receive both timestamps; accepted updates preserve created_at and advance updated_at. Callers cannot supply either field. |
| `contract.rtg.controller.data_object_write_semantics` | `ApplyLiveGraphChanges` | `controller.applyLiveGraphChanges` | Every controller-mediated data-object write explicitly selects merge or replace. Merge is valid for inserts and partial updates, preserves omitted properties, and rejects expectedVersion. Replace is valid only for an existing object with expectedVersion, treats supplied properties as complete, and removes omitted properties. The controller never infers a missing mode. |
| `contract.rtg.controller.optimistic_concurrency` | `ApplyLiveGraphChanges` | `controller.applyLiveGraphChanges` | While holding the serialized write lane and before any mutation, recompute every replace target's opaque version from the complete current data-object record and ordered direct-anchor associations. If any expectedVersion is stale, reject the whole batch without mutation and return every conflict with the supplied token, winning object, winning associations, and current token. One of two interleaved replace writers sharing a starting token may apply, but both cannot. Token validity is independent of ledger persistence. |
...
| `contract.rtg.controller.cutover_flow` | `ApplyMigrationCutover` | `controller.applyMigrationCutover` | Read the selected migration membership and reviewed schema-evolution ops, reject any uncovered schema diff, snapshot coordinated state, apply reviewed data effects and make-live/make-non-live changes in reference-safe order, validate the result, and either commit an applied status or restore the snapshot and record failed status. |
| `contract.rtg.controller.cutover_sequence` | `ApplyMigrationCutover` | `controller.applyMigrationCutover` | Under the write lock: derive the plan from the selected migration; prove its reviewed ops exactly cover property diffs; validate a projection with reviewed data effects; capture a coordinated preimage; apply property rename/delete data effects; make schema and constraint changes, with make-non-live before make-live for a type key; make graph live changes; validate actual state; optionally prune retired records; then remove the completed migration. Any failure restores the preimage, marks the migration failed when safe, and records cutover_failed. |
| `invariant.rtg.controller.schema_evolution_cutover_is_diff_scoped` | `ApplyMigrationCutover` | `controller.applyMigrationCutover` | Cutover accepts schema-evolution ops only from the stored migration plan. Every added, removed, or changed data-object property between paired live and candidate definitions must be covered by exactly the corresponding reviewed add, rename, delete, or retype op, and every reviewed property op must describe an actual diff. Ad hoc ops supplied at cutover and implied but unreviewed property diffs fail before mutation. |
| `invariant.rtg.controller.schema_evolution_data_effects_are_kernel_sequenced` | `ApplyMigrationCutover` | `controller.applyMigrationCutover` | Reviewed rename_property and delete_property effects rewrite every live data object of the target type before schema live-status flips, preserving object identity and anchor associations. Strict preview applies the same effects to isolated state. Failure restores the complete pre-cutover state. |
| `invariant.rtg.controller.schema_evolution_ops_are_per_op_reported` | `ApplyMigrationCutover` | `controller.applyMigrationCutover` | A successful cutover returns details.schema_evolution_diff with ordered results for every reviewed op, including op identity, kind, target, declared data implication, affected count, and mutation state; its batch counts are derived from those results and the same evidence is ledgered and replayable. |
...
| `contract.rtg.controller.read_semantics` | `RtgController` | `controller` | Query defaults to live-only at the controller boundary unless explicit query options override it. Direct object reads do not lifecycle-filter. A data-object read returns its complete ordered direct-anchor set and a stable opaque version token that changes whenever the visible record or associations change, while unrelated writes do not change it; observedLedgerPosition reports independent persisted-ledger context and may be absent. Migration reads are deterministically ordered. All reads queue behind cutover, restore, and replay mutation and never append mutation ledger records. |
...
| `RtgControllerObjectRead` | `attribute` | `object: RtgObject`, `directAnchorRefs[0..*] ordered: Uuid`, `versionToken[0..1]: String`, `observedLedgerPosition[0..1]: Integer` | Data-object reads include their complete ordered direct-anchor set and an opaque version token derived from both visible inputs. Other graph object kinds have neither token nor direct anchors. The observed ledger position is independent audit context. |
| `RtgControllerWriteConflictDetail` | `attribute` | `objectUuid: Uuid`, `expectedVersion: String`, `currentVersion: String`, `currentObject: RtgDataObject`, `currentDirectAnchorRefs[0..*] ordered: Uuid` | Defined by its typed fields and action requirements. |
...
| `RtgControllerWriteConflict` | `attribute` | `message: String`, `transactionId: Uuid`, `conflicts[1..*]: RtgControllerWriteConflictDetail`, `diagnostic[0..1]: RtgDiagnostic` | Defined by its typed fields and action requirements. |
...
|---|---|---|---|
| `ApplyLiveGraphChangesContractVerification` | `ApplyLiveGraphChanges` | `liveMutationFlow`, `liveMutationLane`, `kernelMetaModelApplySemantics`, `dataObjectWriteSemantics`, `optimisticConcurrencySemantics`, `applyLiveGraphChangesFailureSemantics` | `components/rtg/controller/tests/test_rtg_controller_contract.py#ApplyLiveGraphChangesContractVerification` |
...
| `ApplyMigrationCutoverContractVerification` | `ApplyMigrationCutover` | `cutoverFlow`, `cutoverSequence`, `schemaEvolutionCutoverDiffScoped`, `schemaEvolutionDataEffectsSequenced`, `schemaEvolutionOpsPerOpReported`, `applyMigrationCutoverFailureSemantics` | `components/rtg/controller/tests/test_rtg_controller_contract.py#ApplyMigrationCutoverContractVerification` |
