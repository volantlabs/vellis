...
| `contract.rtg.change_validation.data_write_intent` | `ValidateRtgChangeBatch` | `validator.validateBatch` | Every data-object write explicitly selects merge or replace. Missing or unsupported modes, merge with an expected version, replace without an existing target, and replace without an expected version are blocking schema/object findings. Projection preserves omitted existing properties for merge and removes them for replace. Validation does not compare expectedVersion with current state; optimistic-concurrency comparison remains controller sequencing authority. |
| `contract.rtg.change_validation.track_isolation` | `ValidateRtgChangeBatch` | `validator.validateBatch` | Select tracks before track-specific reference validation and projection. schema_object reads and projects graph and schema; constraint_network reads and projects graph, schema, and constraints and invokes query; migration_cutover may read and project all four stores. Malformed unselected sections and unavailable unselected dependencies cannot affect the report. |
| `contract.rtg.change_validation.rule_catalog` | `RtgChangeValidator` | `validator` | Stable blocking codes include schema_object.unknown_type, undeclared_property, property_kind_mismatch, missing_required_property, missing_required_associated_data, link_endpoint_type_invalid, reference_missing, kernel_system_field_write_rejected, event_update_rejected, data_write_mode_missing, data_write_mode_invalid, data_write_mode_conflict, link_kind_missing, link_kind_invalid, provenance_link_delete_rejected, and merge_candidate.identity_match; constraint_network.pattern_unsatisfied, cardinality_out_of_bounds, constraint_target_unknown, constraint_payload_unevaluable; and migration_cutover.reference_missing, wrong_live_state, invalid_status_transition, post_state_invalid, invalid_time_shape_retrofit. schema_object.delete_blast_radius_by_kind and merge_candidate.identity_force_create_override are informational; migration_cutover.replacement_type_mismatch is warning. Each code retains its declared track and severity. |
...
| `RtgIdentityOverride` | `attribute` | `mode: RtgIdentityOverrideMode`, `reason: String`, `criterionKeys[0..*]: String` | Defined by its typed fields and action requirements. |
| `RtgGraphAnchorWrite` | `attribute` | `ref: RtgChangeReference`, `type: String`, `displayName[0..1]: String`, `system: JsonObject`, `identityOverride[0..1]: RtgIdentityOverride` | Defined by its typed fields and action requirements. |
| `RtgGraphDataObjectWrite` | `attribute` | `ref: RtgChangeReference`, `type: String`, `mode[0..1]: RtgDataObjectWriteMode`, `expectedVersion[0..1]: String`, `properties: JsonObject`, `system: JsonObject`, `anchorRefs[1..*]: RtgChangeReference`, `identityOverride[0..1]: RtgIdentityOverride` | mode remains optional in the transport-neutral input so validation can report an omitted mode. merge preserves unspecified existing properties and forbids expectedVersion. replace denotes a complete property set and requires expectedVersion for an existing target. |
...
| `RtgValidationOptions` | `attribute` | `selection: RtgValidationTrackSelection` = `RtgValidationTrackSelection::'all'`, `tracks[0..*]: RtgValidationTrack`, `findingLimit[0..1]: Integer` | Canonical normalized options. all requires an empty tracks collection and executes every track; selected requires a non-empty unique tracks collection and executes exactly those tracks. findingLimit, when present, is positive and limits returned findings only. |
| `RtgValidationFinding` | `attribute` | `track: RtgValidationTrack`, `severity: RtgValidationSeverity`, `code: String`, `message: String`, `suggestion[0..1]: String`, `affectedReferences[0..*]: String`, `diagnostic[0..1]: RtgDiagnostic` | Defined by its typed fields and action requirements. |
...
| `RtgValidationInputInvalid` | `attribute` | `message: String` | Defined by its typed fields and action requirements. |
...
| `RtgIdentityOverrideMode` | `force_create` |
| `RtgDataObjectWriteMode` | `merge`, `replace` |

## Verification

| Verification | Subject | Objectives | Evidence |
|---|---|---|---|
| `ValidateRtgChangeBatchContractVerification` | `ValidateRtgChangeBatch` | `projectionSemantics`, `dataWriteIntentSemantics`, `trackIsolation`, `validateRtgChangeBatchFailureSemantics` | `components/rtg/change_validation/tests/test_rtg_change_validation_contract.py#ValidateRtgChangeBatchContractVerification` |
...
| `RtgChangeValidationBoundaryVerification` | `RtgChangeValidator` | `referenceSemantics`, `validationRuleCatalog`, `validationRules`, `kernelMetaModelValidation`, `reportSemantics`, `validationOptionSemantics`, `intentionalBoundary`, `noMutation`, `deterministicReports`, `findingsComprehensive`, `sourceOfTruthExternal`, `blockingFindingsControlAcceptance`, `findingsAgentActionable`, `tracksExtractable`, `batchSectionsExplicit`, `constraintSchemaCompatibility`, `patternEvaluationDelegated`, `cascadeEffectsFromPreview` | `components/rtg/change_validation/tests/test_rtg_change_validation_contract.py#RtgChangeValidationBoundaryVerification` |
