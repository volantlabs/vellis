...
| `buildCutoverPlan` | `BuildMigrationCutoverPlan` | in `migration: RtgMigrationRecord`; out `plan: RtgMigrationCutoverPlan` | `RtgMigrationRecordInvalid` | Purely copy ordered lifecycle membership, replacement mappings, time-shape retrofits, and reviewed schema-evolution operations from a concrete valid migration record. |
...
| `contract.rtg.migration.cutover_plan` | `BuildMigrationCutoverPlan` | `migration.buildCutoverPlan` | The plan deterministically copies all make-live, make-non-live, replacement, schema time-shape retrofit, and reviewed schema-evolution operation membership without reading or mutating referenced component state. Schema-evolution operation order is preserved. A retrofit has one concrete definition identity and defaults an omitted time shape to state_now. |
| `invariant.rtg.migration.schema_evolution_ops_closed` | `PutMigration` | `migration.putMigration` | Every schema-evolution op has a non-empty unique opId; uses only the declared opKind, targetKind, and dataImplication vocabularies; and supplies exactly the fields required by its op kind. Property ops target data_object, link-type ops target link, propertyKey is present only for property ops, replacementKey only for rename ops, and replacementField only for add_property or retype_property. Invalid input names the offending field and replaces or creates no migration record. |
| `invariant.rtg.migration.schema_evolution_ops_are_review_data` | `RtgMigration` | `migration` | Schema-evolution ops are ordered reviewed migration data. The migration component records and round-trips them but does not infer diffs, mutate schema or graph state, or execute their data implications. |
...
| `RtgSchemaTimeShapeRetrofit` | `attribute` | `definitionUuid: Uuid`, `timeShape[0..1]: RtgSchemaTimeShape` | Omitted timeShape explicitly defaults a legacy node definition to state_now. |
| `RtgSchemaEvolutionOp` | `attribute` | `opId: String`, `opKind: RtgSchemaEvolutionOpKind`, `targetKind: RtgSchemaEvolutionTargetKind`, `targetTypeKey: String`, `propertyKey[0..1]: String`, `replacementKey[0..1]: String`, `replacementField[0..1]: JsonObject`, `sourceDefinitionUuid[0..1]: Uuid`, `candidateDefinitionUuid[0..1]: Uuid`, `dataImplication: RtgSchemaEvolutionDataImplication` | One reviewed, declarative schema change. Shape is determined by opKind; this value contains no executable transform. |
| `RtgMigrationRecord` | `item` | `migrationId[0..1]: String`, `description: String`, `status: RtgMigrationStatus` = `RtgMigrationStatus::draft`, `schemaMakeLive[0..*]: Uuid`, `schemaMakeNonLive[0..*]: Uuid`, `constraintMakeLive[0..*]: Uuid`, `constraintMakeNonLive[0..*]: Uuid`, `graphMakeLive[0..*]: Uuid`, `graphMakeNonLive[0..*]: Uuid`, `schemaReplacements[0..*]: RtgMigrationReplacement`, `constraintReplacements[0..*]: RtgMigrationReplacement`, `graphReplacements[0..*]: RtgMigrationReplacement`, `schemaTimeShapeRetrofits[0..*]: RtgSchemaTimeShapeRetrofit`, `schemaEvolutionOps[0..*] ordered: RtgSchemaEvolutionOp`, `evidence[0..*] ordered: RtgMigrationEvidence`, `metadata: JsonObject` | Identity may be absent only on write. Stored records have a concrete generated or caller-supplied ID. The record stores references and intent, never referenced resources or executable transforms. |
...
| `RtgMigrationCutoverPlan` | `attribute` | `migrationId: String`, `schemaMakeLive[0..*] ordered: Uuid`, `schemaMakeNonLive[0..*] ordered: Uuid`, `constraintMakeLive[0..*] ordered: Uuid`, `constraintMakeNonLive[0..*] ordered: Uuid`, `graphMakeLive[0..*] ordered: Uuid`, `graphMakeNonLive[0..*] ordered: Uuid`, `schemaReplacements[0..*] ordered: RtgMigrationReplacement`, `constraintReplacements[0..*] ordered: RtgMigrationReplacement`, `graphReplacements[0..*] ordered: RtgMigrationReplacement`, `schemaTimeShapeRetrofits[0..*] ordered: RtgSchemaTimeShapeRetrofit`, `schemaEvolutionOps[0..*] ordered: RtgSchemaEvolutionOp` | Defined by its typed fields and action requirements. |
...
| `RtgSchemaEvolutionOpKind` | `add_type`, `rename_type`, `delete_type`, `add_property`, `rename_property`, `retype_property`, `delete_property`, `add_link_type`, `rename_link_type`, `delete_link_type` |
| `RtgSchemaEvolutionTargetKind` | `anchor`, `data_object`, `link` |
| `RtgSchemaEvolutionDataImplication` | `no_existing_data_change`, `rename_existing_values`, `rewrite_existing_type_key`, `strip_existing_values`, `requires_empty_type`, `requires_backfill` |

## Verification

| Verification | Subject | Objectives | Evidence |
|---|---|---|---|
| `PutMigrationContractVerification` | `PutMigration` | `putMigrationEffect`, `membershipSemantics`, `schemaEvolutionOpsClosed`, `putMigrationFailureSemantics` | `components/rtg/migration/tests/test_rtg_migration_contract.py#PutMigrationContractVerification` |
...
| `RtgMigrationBoundaryVerification` | `RtgMigration` | `intentionalBoundary`, `idUnique`, `cutoverSetsDisjoint`, `notNormalCrud`, `candidatesMaterialized`, `schemaEvolutionOpsReviewData`, `referencesAreData`, `noStoreMutation`, `statusTransitionControlled`, `statusTrackingNotProof`, `completedHistoryExternal`, `rollbackForwardChange` | `components/rtg/migration/tests/test_rtg_migration_contract.py#RtgMigrationBoundaryVerification` |
