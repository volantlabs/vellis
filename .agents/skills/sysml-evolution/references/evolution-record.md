...
classification, implementation status, nearest plausible wrong system, compatibility effect,
disposition, and optional work-item owner. Classify by the needed response:
...
completion owner, and evidence intent that names the nearest wrong realization. Status and evidence
advance during execution; the selected meaning and owner are planning state.
...
Keep authority coverage separate from implementation status. A work item may cite partial authority
coverage if obligations remain elsewhere, and it must name the remaining work. Use implementation
status `not evaluated`, `absent`, `partial`, `conforming`, or `conflicting`.
