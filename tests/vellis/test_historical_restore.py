...
def test_the_owner_can_reach_restoration_from_the_command_line(tmp_path: Path) -> None:
    """A capability nobody can invoke is not a deferred one.

    The ten agent tools leave restoration out deliberately, because that surface decides
    no authorization and an agent that damaged a memory should not also hold the means of
    rewriting it. That reasoning gives the owner the handle; it does not take it away from
    everyone. Declining leaves the memory exactly as it was, which is the half an owner has
    to be able to rely on before they will ever use the other half.
    """
    import io

    from vellis import __main__ as entry

    system = RTGSystem.open(tmp_path / "vellis.sqlite3")
    try:
        assert system.initialize_fresh(
            build_rich_definitions(), provenance=_owner(), initialization_summary="fresh"
        ).accepted
        assert system.apply_graph_change(
            GraphChange(anchor_upserts=(Anchor("a-1", "person", "Ada"),)), provenance=_owner()
        ).accepted
    ...
    assert (
        entry.main(
            ["restore", "--data-dir", str(tmp_path), "--revision", "0"],
            stdout=declined,
            confirm=lambda _: "n",
        )
        == entry.EXIT_SUCCESS
    )
    assert "nothing was restored" in declined.getvalue()
    reopened = RTGSystem.open(tmp_path / "vellis.sqlite3")
    try:
        assert reopened.store.current_revision() == before
    finally:
        reopened.close()
    ...
    assert (
        entry.main(
            ["restore", "--data-dir", str(tmp_path), "--revision", "0", "--yes"],
            stdout=accepted,
        )
        == entry.EXIT_SUCCESS
    )
    assert "restored" in accepted.getvalue()
    restored = RTGSystem.open(tmp_path / "vellis.sqlite3")
    try:
        # Forward, not back: the restoration is itself a new revision.
        assert restored.store.current_revision() == before + 1
        assert materialize_state(restored).graph.anchors == ()
    finally:
        restored.close()
