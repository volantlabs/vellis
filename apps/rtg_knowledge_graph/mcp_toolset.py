...
def _response_options(value: dict[str, Any] | None) -> dict[str, str]:
    ...
    format_value = value.get("format", "properties_only")
...
def _shape_persisted_snapshot_result(
    ...
    *,
    ...
    if isinstance(snapshot, dict):
        ...
        counts = encoded.get("state_counts")
        encoded["summary"] = {
            "relative_path": relative_path,
            "size_bytes": encoded.get("size_bytes"),
            "digest": encoded.get("digest"),
            "state_counts": dict(counts) if isinstance(counts, dict) else {},
        }
